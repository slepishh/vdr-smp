...
document.addEventListener('DOMContentLoaded', () => {
    function resizeVideoBg(){
        if($('.js-video').attr('data-video') !== undefined){
            $('.js-video').addClass('video');
            let prp_v = 0.3877;
            if ($('.js-video').hasClass('js-videoAbout')){
                prp_v = 0.27552;
            }
            let video;
            if($('.js-video video').length >= 1){
                video = $('.js-video video');
            } else {
                video = $('<video autoplay="true" loop="true" preload="auto" muted="muted"><source src="'+$('.js-video').attr('data-video')+'.webm" type="video/webm"><source src="'+$('.js-video').attr('data-video')+'.mp4" type="video/mp4"><source src="'+$('.js-video').attr('data-video')+'.ogv" type="video/ogg"></video>');
            }
            let prp = $(".js-video").outerHeight()/$(".js-video").outerWidth();
            let width,left,top,height = 0;
            if(prp > prp_v) {
                video.css("width",$(".js-video").outerHeight()/prp_v);
                video.css("left",($(".js-video").outerWidth()-$(".js-video video").width())/2);
                video.css("top",0);
            } else {
                video.css("width",'auto');
                video.css("height",$(".js-video").width()*prp_v);
                video.css("top",($(".js-video").outerHeight()-$(".js-video video").height())/2);
                video.css("left",0);
            }
            $('.js-video').prepend(video);
        }
    }
